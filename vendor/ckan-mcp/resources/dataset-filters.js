...
const buildFormatFilter = (rawFormat) => {
    ...
    const clauses = fields.flatMap((field) => variants.map((value) => `${field}:"${escapeSolrTerm(value)}"`));
    return `(${clauses.join(" OR ")})`;
};
const registerDatasetFilterResource = (server, config) => {
    server.registerResource(config.name, new ResourceTemplate(config.template, { list: undefined }), {
        title: config.title,
...
export function registerGroupDatasetsResource(server) {
...
export function registerOrganizationDatasetsResource(server) {
    registerDatasetFilterResource(server, {
...
export function registerTagDatasetsResource(server) {
    registerDatasetFilterResource(server, {
...
export function registerFormatDatasetsResource(server) {
    registerDatasetFilterResource(server, {
