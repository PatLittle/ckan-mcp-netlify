 ...
 */
export function parseCkanUri(uri) {
    ...
    if (pathParts.length < 2) {
        throw new Error(`Invalid ckan:// URI: expected /{type}/{id}, got ${uri.pathname}`);
    }
 ...
 */
export function parseCkanPath(uri) {
    const hostname = uri.hostname;
    if (!hostname) {
        throw new Error("Invalid ckan:// URI: missing server hostname");
    }
 ...
 */
export function isValidCkanServer(hostname) {
