 ...
 */
export function hasExplicitBooleanOperator(query) {
    ...
    if (BOOL_KEYWORD_PATTERN.test(trimmed))
        return true;
 ...
 */
export function mayNeedTextWrapping(query) {
    const trimmed = query.trim();
    return (trimmed !== DEFAULT_SEARCH_QUERY &&
...
function isEscaped(query, offset) {
    let backslashes = 0;
    for (let i = offset - 1; i >= 0 && query[i] === "\\"; i--)
        backslashes++;
 ...
 */
export function escapeForTextWrapping(query) {
    const keepGroups = hasBalancedParens(query);
    return query.replace(SOLR_SPECIAL_CHARS, (char, offset, whole) => {
        ...
        // the backslash.
        const atTermStart = offset === 0 || /\s/.test(previous) || (keepGroups && previous === "(");
