...
export function registerAnalyzeTools(server) {
    server.registerTool("ckan_analyze_datasets", {
...
Typical workflow: ckan_analyze_datasets → ckan_datastore_search (with known field names)`,
        ...
        try {
            const searchResult = await makeCkanRequest(params.server_url, 'package_search', { q: params.q, rows: params.rows });
            ...
            const analyzed = await Promise.all(datasets.map(async (dataset) => {
                ...
                for (const resource of resources) {
                    if (resource.datastore_active === true) {
                        try {
                            const schema = await makeCkanRequest(params.server_url, 'datastore_search', { resource_id: resource.id, limit: 0 });
...
export function formatCatalogStatsMarkdown(serverUrl, total, facets) {
    const LABELS = {
        groups: "Categories",
        res_format: "Formats",
...
export function registerCatalogStatsTools(server) {
    server.registerTool("ckan_catalog_stats", {
