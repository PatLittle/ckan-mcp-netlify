...
Typical workflow: ckan_group_list → ckan_group_show (inspect one) → ckan_package_search with fq="groups:name" (browse its datasets)`,
        ...
        try {
            ...
            const result = await makeCkanRequest(params.server_url, 'group_list', {
                ...
                if (params.all_fields) {
                    for (const group of result) {
                        ...
                        if (group.description)
                            markdown += `- **Description**: ${sanitizeInline(group.description.substring(0, 200))}\n`;
                    ...
                    markdown += result.map((name) => `- ${sanitizeInline(name)}`).join('\n');
                }
            }
            return {
    ...
    server.registerTool("ckan_group_show", {
...
Typical workflow: ckan_group_show → ckan_package_show (inspect a dataset) → ckan_datastore_search (query its data)`,
        ...
        try {
            const result = await makeCkanRequest(params.server_url, 'group_show', {
                id: params.id,
    ...
    server.registerTool("ckan_group_search", {
