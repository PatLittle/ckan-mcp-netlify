...
export function registerTagTools(server) {
    server.registerTool("ckan_tag_list", {
...
Typical workflow: ckan_tag_list → ckan_package_search with fq="tags:tag_name" (find datasets by tag) → ckan_package_show`,
        ...
        try {
            ...
            if (params.tag_query) {
                ...
                const matching = (candidates) => candidates.filter(tag => stripAccents(tag.name.toLowerCase()).includes(needle));
                tags = matching(tags);
                ...
                if (tags.length < params.limit) {
                    ...
                    const wide = await makeCkanRequest(params.server_url, 'package_search', { ...apiParams, 'facet.limit': -1 });
            ...
            if (params.fq)
                markdown += `**Filter**: ${params.fq}\n`;
            if (params.tag_query)
                markdown += `**Tag Query**: ${params.tag_query}\n`;
