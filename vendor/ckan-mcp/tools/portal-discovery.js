...
function formatMarkdown(portals, total, limit) {
    if (portals.length === 0)
        return "No CKAN portals found matching the given filters.";
    const rows = portals.map(p => `| [${p.SiteInfo.site_title || p.Href}](${p.Href}) | ${p.Coordinates.country_name} | ${p.Version} | ${p.DatasetsNumber.toLocaleString()} | ${p.SiteInfo.locale_default} | ${(p.Plugins || []).includes("datastore") ? "✅" : "❌"} |`).join("\n");
    return `# CKAN Portals
...
export function registerPortalDiscoveryTools(server) {
    server.registerTool("ckan_find_portals", {
